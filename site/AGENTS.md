...
- Hero recap entrance: keep the full event recap visible in the first-screen Hero as a large, book-like editorial object rather than a small button. It opens the first-party `/recap` page, remains visually prominent on desktop and mobile, and localizes its cover/CTA copy in Chinese, Thai, and English.
- Integrated recap architecture: the Hero book and shared navigation open the long-form recap as the first-party `/recap` route on the original activity domain. The recap reuses the main CMI header and footer, contains only the designed editorial article, and must not expose or link to the WeChat-safe publishing variant. The entire article—including body copy, captions, methods, archive CTAs, and museum route cards—must remain complete in Chinese, Thai, and English at `/recap`, `/recap?lang=th`, and `/recap?lang=en`.
...
- The event-ended control remains visible but disabled. Keep the CMI official-account QR and the WeChat contact `LinkLinkGuan` directly visible beneath it in the Hero; do not hide this follow-up information in a modal or restore the expired activity-group QR.
- The second-screen title must explicitly identify the activity outcomes/results, and the Hero recap book must include a plain-language hint that clicking it reveals what happened during the event.
